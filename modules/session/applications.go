...
func (self *SessionPlugin) GetAppList() []Application{
  ...
  for _, i := range files {
    ...
    app.OnlyShowIn      = compact(Stosl(file["Desktop Entry"]["OnlyShowIn"]))
    app.NotShowIn       = compact(Stosl(file["Desktop Entry"]["NotShowIn"]))
    app.Actions         = compact(Stosl(file["Desktop Entry"]["Actions"]))
    app.MimeType        = compact(Stosl(file["Desktop Entry"]["MimeType"]))
    app.Categories      = compact(Stosl(file["Desktop Entry"]["Categories"]))
    app.Implements      = compact(Stosl(file["Desktop Entry"]["Implements"]))
    app.Keywords        = compact(Stosl(file["Desktop Entry"]["Keywords"]))
